...
New to DraftForge or preparing for a real draft? For the September 9, 2026 “It’s Fun To Do Bad Things” salary-cap draft, use the focused [September 9 auction runbook](docs/september-9-auction-runbook.md). The general operator flow is in the [day-one draft canvas](DRAFT_DAY_OPERATOR_CANVAS.md), and [DRAFT_DAY_HANDOVER.md](DRAFT_DAY_HANDOVER.md) contains the full release gates and certification ledger. Moving the project to another computer or Codex account? Start with [MIGRATION.md](MIGRATION.md) and [AGENT_HANDOFF.md](AGENT_HANDOFF.md).
...
The current packaged candidate is companion v0.2.32: unpacked-source SHA-256 `2dabe8db89c5dab6b00730120b06a7e75ccf8b30e92301e5e38fc699bc74560b`, ZIP SHA-256 `b59c6a9ac67f27cbeea97c54a3d702bb223b39e355c879a0a10af4cd2cae93c2`, 19 source files. The full test suite passes 689/689 after typecheck and production build, including 20 complete deterministic snake drafts (2,560 picks) and 20 complete deterministic salary-cap drafts (2,560 sales). The visual certification passes 11/11 states, including the narrowest three-column breakpoint with a classic scrollbar gutter, and the prior v0.2.31 latency, contention, load, and live-control measurements below remain historical mechanics evidence because v0.2.32 changes only companion ownership and recovery boundaries. `npm audit` reports zero vulnerabilities. These automated runs certify local mechanics, not current player advice or an authenticated ESPN room.

Current authenticated arming is intentionally **NO-GO until the time-bound September 9 gates run**. On the certified Mac, the Tradyr credential is stored in the native Keychain and injected only into the production server process; it is never printed, logged, committed, placed in a URL, or exposed to Chrome. The source adapter requests one atomic keyed board with `limit=1000&offset=0` and rejects capped, incomplete, mismatched-profile, duplicate, or stale responses. A 2026-08-31 exact-profile warm returned 5/5 for PPR/12-team/2026/two-QB, and the v0.2.32 companion, authenticated league import, 500-player status pool, and no-reinstall recovery path have been proven on this Mac. The active audit is deliberately invalid after a production-server restart and must be republished from the exact dashboard before arming. The exact salary-cap target is “It’s Fun To Do Bad Things XVII” (league `44050`, team `7`, season `2026`); use [the focused runbook](docs/september-9-auction-runbook.md). The final authenticated pre-room import, current source/availability refresh, and live-room no-click gate cannot be completed before those time-bound inputs exist. Official availability news remains a separate short-lived hold/veto overlay and never changes consensus scores or weights.
...
For local development after the companion has been loaded once from this repository's `extension/` directory, opening `http://localhost:3000/?reloadCompanion=1` performs the complete bounded update flow: it reloads the installed unpacked companion from disk, reloads the dashboard, and reloads the single managed ESPN tab. The command is rejected from non-localhost pages, from an active salary-cap lifecycle, and when the ESPN target is ambiguous. Never invoke it during an active action window.

Do **not** reinstall the companion for an invalidated content-script context, expired writer lease, or ordinary source update. DraftForge uses a versioned bridge addressed to one exact Chrome runtime, ignores legacy bridge traffic, blocks if two current runtimes answer, and attempts one exact in-place lease recovery while idle. The recovery never retries a pick, nomination, or bid, and Auto-Draft remains off until the current checklist passes. A reinstall is reserved only for the true first installation or a missing/corrupt Chrome extension entry.
