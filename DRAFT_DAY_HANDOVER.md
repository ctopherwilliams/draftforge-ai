...
This is the operational source of truth for running an ESPN draft with Codex as the strategy/status cockpit, DraftForge as the command center and sole production writer, and Chrome as the execution surface. The current event-specific priority is the September 8, 2026 salary-cap draft documented in [docs/september-9-auction-runbook.md](docs/september-9-auction-runbook.md); that runbook supersedes snake rehearsal work for this event. Tony Pollard ($0) and Terry McLaurin ($1) are the locked keepers, so the authenticated opening state must be $199 and 12 open roster slots. New operators should first read [DRAFT_DAY_OPERATOR_CANVAS.md](DRAFT_DAY_OPERATOR_CANVAS.md). Read this handover before every rehearsal and real draft. Update the **Certification ledger** and **Known launch risks** whenever a rehearsal changes the evidence.
